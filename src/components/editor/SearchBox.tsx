...
export default function SearchBox({ onPick }: { onPick: (name: string, lng: number, lat: number) => void }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => void fetch(`/api/search?q=${encodeURIComponent(q.trim())}`).then((response) => response.ok ? response.json() as Promise<{ pois: Poi[] }> : { pois: [] }).then((data) => { setResults(data.pois ?? []); setOpen(true); }).catch(() => setResults([])), 300);
